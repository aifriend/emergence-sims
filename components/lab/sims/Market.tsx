...
type MarketP = { chartFrac: number; gc: number; noise: number; speed: number };

const PF = 0; // log fundamental value (constant ⇒ price near-stationary, returns are the object)
const WIN = 400; // rolling display window (steps)
...
const CRASH = 0.06; // |return| flagged as a crash flash (log units)
...
/** one discrete market step. `bias` (0..1) tilts the steady-state chartist share. */
function advance(s: Sim, P: MarketP): number {
  const { gc, noise } = P;
  ...
  const pNext = s.p + MU * (nf * df + s.nc * dc) + eps;
